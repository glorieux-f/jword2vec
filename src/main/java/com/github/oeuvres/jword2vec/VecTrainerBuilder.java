...
/**
 * Builder pattern for training a new {@link VecModel}
 * <p>
 ...
 * <ul>
 * <li>The original version does a reduction step when learning the vocabulary
 * from the file when the vocab size hits 21 million words, removing any words
 * that do not meet the minimum frequency threshold. This Java port has no such
 * reduction step.
 * <li>The original version injects a &lt;/s&gt; token into the vocabulary (with
 * a word count of 0) as a substitute for newlines in the input file. This Java
 * port's vocabulary excludes the token.
 * </ul>
 * <li>In partitioning the file for processing
 * <ul>
 * <li>The original version assumes that sentences are delimited by newline
 * characters and injects a sentence boundary per 1000 non-filtered tokens, i.e.
 * valid token by the vocabulary and not removed by the randomized sampling
 * process. Java port mimics this behavior for now ...
 * <li>When the original version encounters an empty line in the input file, it
 * re-processes the first word of the last non-empty line with a sentence length
 * of 0 and updates the random value. Java port omits this behavior.
 * </ul>
 * <li>In the sampling function
 * <ul>
 * <li>The original C documentation indicates that the range should be between 0
 * and 1e-5, but the default value is 1e-3. This Java port retains that
 * confusing information.
 * <li>The random value generated for comparison to determine if a token should
 * be filtered uses a float. This Java port uses double precision for twice the
 * fun
 * </ul>
 * <li>In the distance function to find the nearest matches to a target query
 * <ul>
 * <li>The original version includes an unnecessary normalization of the vector
 * for the input query which may lead to tiny inaccuracies. This Java port
 * foregoes this superfluous operation.
 * <li>The original version has an O(n * k) algorithm for finding top matches
 * and is hardcoded to 40 matches. This Java port uses Google's lovely
 * {@link com.google.common.collect.Ordering#greatestOf(java.util.Iterator, int)}
 ...
 * <p>
 * <p>
 * Please do not hesitate to peek at the source code. <br>
 * It should be readable, concise, and correct. <br>
 * I ask you to reach out if it is not.
 */
public class VecTrainerBuilder
{
     ...
     * Defaults to 100
     */
    public VecTrainerBuilder setLayerSize(int layerSize)
    {
     ...
     */
    public VecTrainerBuilder setWindowSize(int windowSize)
    {
     ...
     */
    public VecTrainerBuilder useNumThreads(int numThreads)
    {
     ...
     */
    public VecTrainerBuilder type(NeuralNetworkType type)
    {
     ...
     */
    public VecTrainerBuilder useHierarchicalSoftmax()
    {
    ...
    /**
     * Number of negative samples to use Common values are between 5 and 10
     * <p>
     * Defaults to 0
     */
    public VecTrainerBuilder useNegativeSamples(int negativeSamples)
    {
     ...
     * <p>
     * If this is not specified, word2vec will attempt to learn a vocabulary from
     * the training data
     * 
     * @param vocab {@link Map} from token to frequency
     */
    public VecTrainerBuilder useVocab(Multiset<String> vocab)
    {
    ...
    /**
     * Specify the minimum frequency for a valid token to be considered part of the
     * vocabulary
     * <p>
     * Defaults to 5
     */
    public VecTrainerBuilder setMinVocabFrequency(int minFrequency)
    {
     ...
     */
    public VecTrainerBuilder setInitialLearningRate(double initialLearningRate)
    {
     ...
     * frequency in the training data, e.g. stopwords, will be randomly removed
     * <p>
     * Default is 1e-3, useful range is (0, 1e-5)
     */
    public VecTrainerBuilder setDownSamplingRate(double downSampleRate)
    {
    ...
    public VecTrainerBuilder setNumIterations(int iterations)
    {
    ...
    public VecTrainerBuilder setListener(TrainingProgressListener listener)
    {
    ...
    /** Train the model */
    public VecModel train(Iterable<List<String>> sentences) throws InterruptedException
    {
        ...
        Optional<Multiset<String>> vocab = this.vocab == null ? Optional.<Multiset<String>>absent()
                : Optional.of(this.vocab);

        return new VecTrainer(minFrequency, vocab, new NeuralNetworkConfig(type, numThreads, iterations, layerSize,
                windowSize, negativeSamples, downSampleRate, initialLearningRate, useHierarchicalSoftmax))
    ...
    public interface TrainingProgressListener
    {
         ...
         * @param progress Progress of the current stage as a double value between 0 and
         *                 1
         */
