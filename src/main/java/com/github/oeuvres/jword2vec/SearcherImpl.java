...
import com.github.oeuvres.jword2vec.util.Pair;
...
class SearcherImpl implements Searcher
{
    ...
    {
        // loop on all vectors, if distance < max of seen according to limit, collect
    ...
    private double calculateDistance(double[] vec1, double[] vec2)
    {
        ...
        for (int a = 0; a < model.layerSize; a++) {
            // TODO, NaN bug in loading
            if ( Double.isNaN(vec1[a]) || Double.isNaN(vec2[a])) continue;
            d += vec2[a] * vec1[a];
        }
        return d;
    }
